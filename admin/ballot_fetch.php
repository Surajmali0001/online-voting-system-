<?php
	...
	while($row = $query->fetch_assoc()){
		...
		while($crow = $cquery->fetch_assoc()){
			...
			$candidate .= '
				<li>
					'.$input.'<img src="'.$image.'" height="100px" width="100px" class="clist"><span class="cname clist"><b>'.$crow['firstname'].' '.$crow['lastname'].'</span></b>
				</li>
			';
		}

		$instruct = ($row['max_vote'] > 1) ? 'You may select up to '.$row['max_vote'].' candidates' : '<center><b>Select only one candidate</center></b>';
